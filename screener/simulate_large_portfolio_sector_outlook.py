"""Bot#12: same "large" capital-constrained mechanics as simulate_large_portfolio.py (Bot#3 --
no max-per-sector cap, least-held-sector-first allocation, reinforcement when a sector runs dry
of fresh candidates), same ranking (select_top_picks.composite_score, unchanged), except a
candidate whose sector is currently rated "sous_pression" in data/universe/sector_outlook.csv --
...
LEDGER_PATH = HERE / "results/simulation/large_portfolio_ledger_sector_outlook.csv"
STATE_PATH = HERE / "results/simulation/large_state_sector_outlook.json"
...
SUMMARY_PATH = HERE / "results/simulation/large_summary_sector_outlook.json"
EQUITY_CURVE_PATH = HERE / "results/simulation/large_equity_curve_sector_outlook.csv"
...
def fill_slots(ledger: pd.DataFrame, candidates: pd.DataFrame, valuation: pd.DataFrame, cash: float, today: str,
    ...
    # all_sectors (used for the least-held-first round-robin) deliberately still comes from
    # the FULL candidates frame, not the veto-filtered pool: a pressured sector should still
    # get its turn in the rotation and fall through to "reinforce an existing position" or
    # "exhausted", exactly like a sector that's simply out of fresh candidates today -- not be
    # silently removed from consideration the way it is in pool.
...
def write_summary(ledger: pd.DataFrame, cash: float):
    ...
    print(f"\n=== Portefeuille large (veto sectoriel) : {summary['nb_open']} positions, "
