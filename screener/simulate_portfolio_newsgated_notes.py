"""Bot#13 (blind + filtre actu Ollama + seuil notes): same "buy every LONG candidate, no
ranking, no capital constraint" mechanics as simulate_portfolio.py (Bot#1), combining the two
independent gates already used separately by Bot#4 (news_filter.news_verdict, see
simulate_portfolio_newsgated.py) and Bot#7 (notes threshold, see simulate_portfolio_notes.py).
Neither gate touches the other's logic (one is a pre-fetch notes threshold, the other a
post-fresh-check Ollama call) -- combining them is a straight AND of both, checked cheapest
first: notes threshold (free, in-memory) before the fresh fetch, Ollama news verdict (slowest)
last, exactly the ordering Bot#4 and Bot#7 already use individually.
...
LEDGER_PATH = HERE / "results/simulation/portfolio_ledger_newsgated_notes.csv"
...
SUMMARY_PATH = HERE / "results/simulation/summary_newsgated_notes.json"
EQUITY_CURVE_PATH = HERE / "results/simulation/equity_curve_newsgated_notes.csv"
...
def open_new_positions(ledger: pd.DataFrame, candidates: pd.DataFrame, valuation: pd.DataFrame,
    ...
    if new_rows:
        ...
        print(f"{len(new_rows)} nouvelle(s) position(s) ouverte(s) (actu+notes) : {sorted(new_tickers)}")
...
def write_summary(ledger: pd.DataFrame):
    ...
    print(f"\n=== Resume simulation (Bot#13, actu+notes) : {summary['nb_open']} ouvertes, "
