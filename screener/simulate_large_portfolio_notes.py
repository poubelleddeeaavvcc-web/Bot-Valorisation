"""Bot#9: same "large" capital-constrained mechanics as simulate_large_portfolio.py (Bot#3 --
no max-per-sector cap, least-held-sector-first allocation, reinforcement when a sector runs dry
of fresh candidates), except within-sector candidate ranking uses the 2-note quality/perspective
system (screener/quality_perspective_notes.py) instead of select_top_picks.composite_score --
the same substitution Bot#8 makes on top of Bot#2's mechanics, applied here to Bot#3's instead.

Candidates with no computable note (NaN on either pillar) are dropped from the pool entirely
before the per-sector sort, not ranked last -- same rule as Bot#8, for the same reason: buying a
candidate this bot couldn't actually score wouldn't test the notes' predictive value.

Everything else -- FX conversion, fractional-share eligibility, reinforcement bookkeeping,
...
LEDGER_PATH = HERE / "results/simulation/large_portfolio_ledger_notes.csv"
STATE_PATH = HERE / "results/simulation/large_state_notes.json"
...
SUMMARY_PATH = HERE / "results/simulation/large_summary_notes.json"
EQUITY_CURVE_PATH = HERE / "results/simulation/large_equity_curve_notes.csv"
...
def fill_slots(ledger: pd.DataFrame, candidates: pd.DataFrame, valuation: pd.DataFrame, cash: float, today: str,
    ...
    pool = pool.dropna(subset=["notes_score"])  # see module docstring: unscored candidates are
    # excluded from this bot's pool entirely, not ranked last
...
def write_summary(ledger: pd.DataFrame, cash: float):
    ...
    print(f"\n=== Portefeuille large (selection par notes) : {summary['nb_open']} positions, "
