"""Bot#16 (blind + filtre actu Ollama + veto sectoriel newsletters): same "buy every LONG
candidate, no ranking, no capital constraint" mechanics as simulate_portfolio.py (Bot#1),
combining the two independent gates already used separately by Bot#4 (news_filter.news_verdict,
see simulate_portfolio_newsgated.py) and Bot#10 (sector-pressure veto, see
simulate_portfolio_sector_outlook.py). The sector check is a free in-memory pool filter, applied
before the fresh fetch; the Ollama news verdict is the slowest gate, applied last -- same
cheapest-first ordering as every other combined bot in this family.
...
LEDGER_PATH = HERE / "results/simulation/portfolio_ledger_newsgated_sector_outlook.csv"
...
SUMMARY_PATH = HERE / "results/simulation/summary_newsgated_sector_outlook.json"
EQUITY_CURVE_PATH = HERE / "results/simulation/equity_curve_newsgated_sector_outlook.csv"
...
def open_new_positions(ledger: pd.DataFrame, candidates: pd.DataFrame, valuation: pd.DataFrame,
    ...
    if new_rows:
        ...
        print(f"{len(new_rows)} nouvelle(s) position(s) ouverte(s) (actu+veto sectoriel) : {sorted(new_tickers)}")
...
def write_summary(ledger: pd.DataFrame):
    ...
    print(f"\n=== Resume simulation (Bot#16, actu+veto sectoriel) : {summary['nb_open']} ouvertes, "
