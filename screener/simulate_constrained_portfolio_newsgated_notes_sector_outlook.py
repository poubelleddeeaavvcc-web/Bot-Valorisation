"""Bot#23: same capital-constrained paper-trading mechanics as simulate_constrained_portfolio.py
(Bot#2 -- capital, slots, sector cap, NA cap, FX and fractional-share handling all identical),
combining all three independent improvements used separately by Bot#5 (news_filter gate +
customer-concentration sizing), Bot#8 (ranking by notes_score) and Bot#11 (sector-pressure
exclusion). None touch each other's logic: sector veto narrows the pool, notes_score ranks what
remains, the news gate + concentration sizing decides whether/how big to buy whichever candidate
the ranking reaches first in the picking loop.
...
LEDGER_PATH = HERE / "results/simulation/constrained_portfolio_ledger_newsgated_notes_sector_outlook.csv"
STATE_PATH = HERE / "results/simulation/constrained_state_newsgated_notes_sector_outlook.json"
...
SUMMARY_PATH = HERE / "results/simulation/constrained_summary_newsgated_notes_sector_outlook.json"
EQUITY_CURVE_PATH = HERE / "results/simulation/constrained_equity_curve_newsgated_notes_sector_outlook.csv"
...
def write_summary(ledger: pd.DataFrame, cash: float):
    ...
    print(f"\n=== Portefeuille contraint (actu+notes+veto sectoriel) : {summary['nb_open']} positions, "
