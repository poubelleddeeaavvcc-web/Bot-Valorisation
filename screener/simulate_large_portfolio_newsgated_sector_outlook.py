"""Bot#18: same "large" capital-constrained mechanics as simulate_large_portfolio.py (Bot#3 --
no max-per-sector cap, least-held-sector-first allocation, reinforcement when a sector runs dry
of fresh candidates), same ranking (select_top_picks.composite_score, unchanged), combining the
two independent improvements already used separately by Bot#6 (news_filter gate +
customer-concentration sizing) and Bot#12 (sector-pressure exclusion from the pool, see
...
rotation, is still built from the FULL candidates frame, not the veto-filtered pool -- a
pressured sector still gets its turn and falls through to reinforcement/exhaustion, rather than
silently vanishing from the round-robin the way it does from the scoring pool.
...
LEDGER_PATH = HERE / "results/simulation/large_portfolio_ledger_newsgated_sector_outlook.csv"
STATE_PATH = HERE / "results/simulation/large_state_newsgated_sector_outlook.json"
...
SUMMARY_PATH = HERE / "results/simulation/large_summary_newsgated_sector_outlook.json"
EQUITY_CURVE_PATH = HERE / "results/simulation/large_equity_curve_newsgated_sector_outlook.csv"
...
def fill_slots(ledger: pd.DataFrame, candidates: pd.DataFrame, valuation: pd.DataFrame, cash: float, today: str,
    ...
    # candidates (pool, already filtered for the sector veto) AND currently-open positions,
    # since a sector with no fresh candidate left gets REINFORCED (an existing open position,
    # not pool) instead -- see module docstring -- and that path calls news_verdict() too.
    # Added 2026-09-07.
...
def write_summary(ledger: pd.DataFrame, cash: float):
    ...
    print(f"\n=== Portefeuille large (actu+veto sectoriel) : {summary['nb_open']} positions, "
