"""Bot#28: same conviction-averaging mechanics as simulate_delta_portfolio.py (Bot#25, "Delta"
base), combining the two independent improvements already used separately by Bot#26 (ranking by
notes_score, see simulate_delta_portfolio_notes.py) and Bot#27 (sector-pressure exclusion, see
simulate_delta_portfolio_sector_outlook.py) -- the same combination Bot#20 makes on top of
Bot#2's mechanics. Both narrow/reorder the pool before the picking loop and don't touch each
other's logic: the sector veto excludes rows first, then notes_score ranks whatever remains.
...
"""
# Alias lisibilite (mapping perso) : D4 -- famille Delta (conviction), variante + notes + veto sectoriel
import json
import math
import pathlib
import sys
...
LEDGER_PATH = HERE / "results/simulation/delta_portfolio_ledger_notes_sector_outlook.csv"
STATE_PATH = HERE / "results/simulation/delta_state_notes_sector_outlook.json"
...
SUMMARY_PATH = HERE / "results/simulation/delta_summary_notes_sector_outlook.json"
EQUITY_CURVE_PATH = HERE / "results/simulation/delta_equity_curve_notes_sector_outlook.csv"
...
def write_summary(ledger: pd.DataFrame, cash: float):
    ...
    print(f"\n=== Portefeuille Delta (notes+veto sectoriel) : {summary['nb_open']} positions "
