"""Bot#19 (blind + seuil notes + veto sectoriel newsletters): same "buy every LONG candidate,
no ranking, no capital constraint" mechanics as simulate_portfolio.py (Bot#1), combining the two
independent gates already used separately by Bot#7 (notes threshold, see
simulate_portfolio_notes.py) and Bot#10 (sector-pressure veto, see
simulate_portfolio_sector_outlook.py). Both are free in-memory checks -- applied before the
fresh fetch, sector check first (cheapest, no join needed), notes threshold second.
...
LEDGER_PATH = HERE / "results/simulation/portfolio_ledger_notes_sector_outlook.csv"
...
SUMMARY_PATH = HERE / "results/simulation/summary_notes_sector_outlook.json"
EQUITY_CURVE_PATH = HERE / "results/simulation/equity_curve_notes_sector_outlook.csv"
...
def open_new_positions(ledger: pd.DataFrame, candidates: pd.DataFrame, valuation: pd.DataFrame,
    ...
    if new_rows:
        ...
        print(f"{len(new_rows)} nouvelle(s) position(s) ouverte(s) (notes+veto sectoriel) : {sorted(new_tickers)}")
...
def write_summary(ledger: pd.DataFrame):
    ...
    print(f"\n=== Resume simulation (Bot#19, notes+veto sectoriel) : {summary['nb_open']} ouvertes, "
