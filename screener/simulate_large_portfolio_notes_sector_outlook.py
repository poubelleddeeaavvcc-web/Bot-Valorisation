"""Bot#21: same "large" capital-constrained mechanics as simulate_large_portfolio.py (Bot#3 --
no max-per-sector cap, least-held-sector-first allocation, reinforcement when a sector runs dry
of fresh candidates), combining the two independent improvements already used separately by
Bot#9 (ranking by notes_score instead of composite_score, see simulate_large_portfolio_notes.py)
and Bot#12 (sector-pressure exclusion from the pool, see
...
"""
# Alias lisibilite (mapping perso) : C4 -- famille Charlie (large), variante + notes + veto sectoriel
import json
import math
import pathlib
import sys
...
LEDGER_PATH = HERE / "results/simulation/large_portfolio_ledger_notes_sector_outlook.csv"
STATE_PATH = HERE / "results/simulation/large_state_notes_sector_outlook.json"
...
SUMMARY_PATH = HERE / "results/simulation/large_summary_notes_sector_outlook.json"
EQUITY_CURVE_PATH = HERE / "results/simulation/large_equity_curve_notes_sector_outlook.csv"
...
def write_summary(ledger: pd.DataFrame, cash: float):
    ...
    print(f"\n=== Portefeuille large (notes+veto sectoriel) : {summary['nb_open']} positions, "
