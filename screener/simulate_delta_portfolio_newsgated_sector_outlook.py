"""Bot#31: same conviction-averaging mechanics as simulate_delta_portfolio.py (Bot#25, "Delta"
base), same ranking (select_top_picks.composite_score, unchanged), combining the two independent
improvements already used separately by Bot#29 (news_filter gate + customer-concentration
sizing) and Bot#27 (sector-pressure exclusion) -- the same combination Bot#17 makes on top of
Bot#2's mechanics. Neither touches the other's logic: the sector veto narrows the pool before
ranking, the news gate decides whether/how big to buy a candidate already reached in the picking
loop.
...
"""
# Alias lisibilite (mapping perso) : D7 -- famille Delta (conviction), variante + Ollama + veto sectoriel
import json
import math
import pathlib
import sys
...
LEDGER_PATH = HERE / "results/simulation/delta_portfolio_ledger_newsgated_sector_outlook.csv"
STATE_PATH = HERE / "results/simulation/delta_state_newsgated_sector_outlook.json"
...
SUMMARY_PATH = HERE / "results/simulation/delta_summary_newsgated_sector_outlook.json"
EQUITY_CURVE_PATH = HERE / "results/simulation/delta_equity_curve_newsgated_sector_outlook.csv"
...
def write_summary(ledger: pd.DataFrame, cash: float):
    ...
    print(f"\n=== Portefeuille Delta (actu+veto sectoriel) : {summary['nb_open']} positions "
