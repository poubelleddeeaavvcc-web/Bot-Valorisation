...
def fill_slots(ledger: pd.DataFrame, candidates: pd.DataFrame, valuation: pd.DataFrame, cash: float, today: str,
    ...
    while len(exhausted) < len(all_sectors):
        ...
        # fresh momentum/valuation gate -- for a new buy, against the candidate's own
        # (possibly stale) screener data as fallback; for a reinforcement, against the
        # already-open position's own entry basis. A position already failing this bar on
        # fresh data shouldn't be bought OR added to (2026-09-02, see simulate_portfolio.py's
        # module docstring for the same-day round-trip bug this fixes).
        if is_new:
