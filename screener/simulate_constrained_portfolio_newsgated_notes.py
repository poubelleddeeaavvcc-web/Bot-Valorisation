"""Bot#14: same capital-constrained paper-trading mechanics as simulate_constrained_portfolio.py
(Bot#2 -- capital, slots, sector cap, NA cap, FX and fractional-share handling all identical),
combining the two independent improvements already used separately by Bot#5 (news_filter gate +
customer-concentration sizing, see simulate_constrained_portfolio_newsgated.py) and Bot#8
(ranking by notes_score instead of composite_score, see simulate_constrained_portfolio_notes.py).
The two don't touch the same thing -- notes changes WHICH candidate gets picked (the ranking),
newsgated decides whether a picked candidate actually gets bought (and how big) -- so combining
them is: rank the pool by notes_score (as Bot#8 does), then apply Bot#5's news gate + sizing to
whichever candidate the notes ranking reaches first.
...
LEDGER_PATH = HERE / "results/simulation/constrained_portfolio_ledger_newsgated_notes.csv"
STATE_PATH = HERE / "results/simulation/constrained_state_newsgated_notes.json"
...
SUMMARY_PATH = HERE / "results/simulation/constrained_summary_newsgated_notes.json"
EQUITY_CURVE_PATH = HERE / "results/simulation/constrained_equity_curve_newsgated_notes.csv"
...
def fill_slots(ledger: pd.DataFrame, candidates: pd.DataFrame, valuation: pd.DataFrame, cash: float, today: str,
    ...
    # to notes-scored candidates at this point. Added 2026-09-07.
...
def write_summary(ledger: pd.DataFrame, cash: float):
    ...
    print(f"\n=== Portefeuille contraint (actu+notes) : {summary['nb_open']} positions, "
