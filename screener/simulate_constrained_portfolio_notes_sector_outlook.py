"""Bot#20: same capital-constrained paper-trading mechanics as simulate_constrained_portfolio.py
(Bot#2 -- capital, slots, sector cap, NA cap, FX and fractional-share handling all identical),
combining the two independent improvements already used separately by Bot#8 (ranking by
notes_score instead of composite_score, see simulate_constrained_portfolio_notes.py) and Bot#11
(sector-pressure exclusion from the pool, see simulate_constrained_portfolio_sector_outlook.py).
Both narrow/reorder the pool before the picking loop and don't touch each other's logic: the
sector veto excludes rows first, then notes_score ranks whatever remains.
...
LEDGER_PATH = HERE / "results/simulation/constrained_portfolio_ledger_notes_sector_outlook.csv"
STATE_PATH = HERE / "results/simulation/constrained_state_notes_sector_outlook.json"
...
SUMMARY_PATH = HERE / "results/simulation/constrained_summary_notes_sector_outlook.json"
EQUITY_CURVE_PATH = HERE / "results/simulation/constrained_equity_curve_notes_sector_outlook.csv"
...
def write_summary(ledger: pd.DataFrame, cash: float):
    ...
    print(f"\n=== Portefeuille contraint (notes+veto sectoriel) : {summary['nb_open']} positions, "
