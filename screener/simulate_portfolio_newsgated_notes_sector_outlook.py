"""Bot#22 (blind + les 3 mecaniques : filtre actu Ollama + seuil notes + veto sectoriel
newsletters): same "buy every LONG candidate, no ranking, no capital constraint" mechanics as
simulate_portfolio.py (Bot#1), combining all three independent gates used separately by Bot#4
(news_filter.news_verdict), Bot#7 (notes threshold) and Bot#10 (sector-pressure veto). None of
the three touch each other's logic, so combining is a straight AND of all three, cheapest first:
sector check and notes threshold (both free, in-memory) before the fresh fetch, Ollama news
verdict (slowest) last -- same ordering as every other combined bot in this family.
...
from screener.simulate_portfolio_notes import QUALITE_THRESHOLD, PERSPECTIVE_THRESHOLD  # noqa: E402
from screener.newsletter_digest import load_pressured_sectors  # noqa: E402
from screener import news_filter  # noqa: E402

LEDGER_PATH = HERE / "results/simulation/portfolio_ledger_newsgated_notes_sector_outlook.csv"
...
SUMMARY_PATH = HERE / "results/simulation/summary_newsgated_notes_sector_outlook.json"
EQUITY_CURVE_PATH = HERE / "results/simulation/equity_curve_newsgated_notes_sector_outlook.csv"
...
def open_new_positions(ledger: pd.DataFrame, candidates: pd.DataFrame, valuation: pd.DataFrame,
    ...
    # cheap-filters-first pre-filter as the loop (sector veto + notes threshold) so this
    # doesn't waste prefetch calls on candidates that would never reach news_verdict anyway.
    # Added 2026-09-07.
    notes_ok = scored[~scored["ticker"].isin(open_tickers) & ~scored["sector"].isin(pressured) &
    ...
    if new_rows:
        ...
        print(f"{len(new_rows)} nouvelle(s) position(s) ouverte(s) (actu+notes+veto sectoriel) : {sorted(new_tickers)}")
...
def write_summary(ledger: pd.DataFrame):
    ...
    print(f"\n=== Resume simulation (Bot#22, actu+notes+veto sectoriel) : {summary['nb_open']} ouvertes, "
