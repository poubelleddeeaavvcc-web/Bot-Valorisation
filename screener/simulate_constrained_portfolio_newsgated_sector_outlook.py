"""Bot#17: same capital-constrained paper-trading mechanics as simulate_constrained_portfolio.py
(Bot#2 -- capital, slots, sector cap, NA cap, FX and fractional-share handling all identical),
same ranking (select_top_picks.composite_score, unchanged), combining the two independent
improvements already used separately by Bot#5 (news_filter gate + customer-concentration sizing,
see simulate_constrained_portfolio_newsgated.py) and Bot#11 (sector-pressure exclusion from the
pool, see simulate_constrained_portfolio_sector_outlook.py). Neither touches the other's logic --
the sector veto narrows the pool before ranking, the news gate decides whether/how big to buy a
candidate already reached in the picking loop.
...
LEDGER_PATH = HERE / "results/simulation/constrained_portfolio_ledger_newsgated_sector_outlook.csv"
STATE_PATH = HERE / "results/simulation/constrained_state_newsgated_sector_outlook.json"
...
SUMMARY_PATH = HERE / "results/simulation/constrained_summary_newsgated_sector_outlook.json"
EQUITY_CURVE_PATH = HERE / "results/simulation/constrained_equity_curve_newsgated_sector_outlook.csv"
...
def fill_slots(ledger: pd.DataFrame, candidates: pd.DataFrame, valuation: pd.DataFrame, cash: float, today: str,
    ...
    # for the sector veto at this point. Added 2026-09-07.
...
def write_summary(ledger: pd.DataFrame, cash: float):
    ...
    print(f"\n=== Portefeuille contraint (actu+veto sectoriel) : {summary['nb_open']} positions, "
