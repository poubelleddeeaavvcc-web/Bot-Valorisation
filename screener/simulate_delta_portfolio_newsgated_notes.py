"""Bot#30: same conviction-averaging mechanics as simulate_delta_portfolio.py (Bot#25, "Delta"
base), combining the two independent improvements already used separately by Bot#29 (news_filter
gate + customer-concentration sizing) and Bot#26 (ranking by notes_score) -- the same combination
Bot#14 makes on top of Bot#2's mechanics. Notes changes WHICH candidate gets picked (the
ranking), newsgated decides whether a picked candidate actually gets bought (and how big): rank
the pool by notes_score, then apply the news gate + sizing to whichever candidate the ranking
reaches first.
...
"""
# Alias lisibilite (mapping perso) : D6 -- famille Delta (conviction), variante + Ollama + notes
import json
import math
import pathlib
import sys
...
LEDGER_PATH = HERE / "results/simulation/delta_portfolio_ledger_newsgated_notes.csv"
STATE_PATH = HERE / "results/simulation/delta_state_newsgated_notes.json"
...
SUMMARY_PATH = HERE / "results/simulation/delta_summary_newsgated_notes.json"
EQUITY_CURVE_PATH = HERE / "results/simulation/delta_equity_curve_newsgated_notes.csv"
...
def write_summary(ledger: pd.DataFrame, cash: float):
    ...
    print(f"\n=== Portefeuille Delta (actu+notes) : {summary['nb_open']} positions "
