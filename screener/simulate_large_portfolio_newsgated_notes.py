"""Bot#15: same "large" capital-constrained mechanics as simulate_large_portfolio.py (Bot#3 --
...
Bot#6 (news_filter gate + customer-concentration sizing, see
simulate_large_portfolio_newsgated.py) and Bot#9 (ranking by notes_score instead of
composite_score, see simulate_large_portfolio_notes.py). Neither touches the other's logic:
notes_score decides which candidate the least-held-sector rotation reaches, the news gate +
sizing decides whether/how big to actually buy or reinforce with it.
...
LEDGER_PATH = HERE / "results/simulation/large_portfolio_ledger_newsgated_notes.csv"
STATE_PATH = HERE / "results/simulation/large_state_newsgated_notes.json"
...
SUMMARY_PATH = HERE / "results/simulation/large_summary_newsgated_notes.json"
EQUITY_CURVE_PATH = HERE / "results/simulation/large_equity_curve_newsgated_notes.csv"
...
def fill_slots(ledger: pd.DataFrame, candidates: pd.DataFrame, valuation: pd.DataFrame, cash: float, today: str,
    ...
    # candidates (pool, already filtered to notes-scored) AND currently-open positions, since a
    # sector with no fresh candidate left gets REINFORCED (an existing open position, not pool)
    # instead -- see module docstring -- and that path calls news_verdict() too. Added 2026-09-07.
...
def write_summary(ledger: pd.DataFrame, cash: float):
    ...
    print(f"\n=== Portefeuille large (actu+notes) : {summary['nb_open']} positions, "
