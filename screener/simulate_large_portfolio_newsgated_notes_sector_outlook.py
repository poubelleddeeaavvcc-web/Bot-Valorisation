"""Bot#24: same "large" capital-constrained mechanics as simulate_large_portfolio.py (Bot#3 --
no max-per-sector cap, least-held-sector-first allocation, reinforcement when a sector runs dry
of fresh candidates), combining all three independent improvements used separately by Bot#6
(news_filter gate + customer-concentration sizing), Bot#9 (ranking by notes_score) and Bot#12
(sector-pressure exclusion). None touch each other's logic: sector veto narrows the pool,
notes_score ranks what remains within each sector, the news gate + concentration sizing decides
whether/how big to buy or reinforce with whichever candidate the least-held-sector rotation
reaches.

Vigilance (see Bot#12's module docstring): all_sectors, which drives the least-held-first
rotation, is still built from the FULL candidates frame, not the veto-filtered pool.
...
"""
# Alias lisibilite (mapping perso) : C8 -- famille Charlie (large), variante + Ollama + notes + veto sectoriel
import json
import math
import pathlib
import sys
...
LEDGER_PATH = HERE / "results/simulation/large_portfolio_ledger_newsgated_notes_sector_outlook.csv"
STATE_PATH = HERE / "results/simulation/large_state_newsgated_notes_sector_outlook.json"
...
SUMMARY_PATH = HERE / "results/simulation/large_summary_newsgated_notes_sector_outlook.json"
EQUITY_CURVE_PATH = HERE / "results/simulation/large_equity_curve_newsgated_notes_sector_outlook.csv"
...
def fill_slots(ledger: pd.DataFrame, candidates: pd.DataFrame, valuation: pd.DataFrame, cash: float, today: str,
    ...
    # candidates (pool, already filtered for the sector veto + notes score) AND currently-open
    # positions, since a sector with no fresh candidate left gets REINFORCED (an existing open
    # position, not pool) instead -- see module docstring -- and that path calls
    # news_verdict() too. Added 2026-09-07.
    ...
    # see module docstring: all_sectors comes from the full candidates frame, not pool
...
def write_summary(ledger: pd.DataFrame, cash: float):
    ...
    print(f"\n=== Portefeuille large (actu+notes+veto sectoriel) : {summary['nb_open']} positions, "
