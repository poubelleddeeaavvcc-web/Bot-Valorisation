"""Bot#26: same conviction-averaging mechanics as simulate_delta_portfolio.py (Bot#25, "Delta"
base) -- same 300 EUR pool, same reinforce_convictions gate, same exit rules -- except new
candidates are ranked by the 2-note quality/perspective system
(screener/quality_perspective_notes.py) instead of select_top_picks.composite_score, the same
substitution Bot#8 makes on top of Bot#2's mechanics (see simulate_constrained_portfolio_notes.py).

Only fill_slots() (which decides what to buy, and here also what "best" means) is duplicated
here with this bot's own files -- reinforce_convictions, recheck_and_exit, FX/fractional-share
handling are all reused directly from simulate_delta_portfolio.py / simulate_constrained_portfolio.py.
"""
# Alias lisibilite (mapping perso) : D2 -- famille Delta (conviction), variante + seuil notes
import json
import math
import pathlib
import sys
...
LEDGER_PATH = HERE / "results/simulation/delta_portfolio_ledger_notes.csv"
STATE_PATH = HERE / "results/simulation/delta_state_notes.json"
...
SUMMARY_PATH = HERE / "results/simulation/delta_summary_notes.json"
EQUITY_CURVE_PATH = HERE / "results/simulation/delta_equity_curve_notes.csv"
...
def write_summary(ledger: pd.DataFrame, cash: float):
    ...
    print(f"\n=== Portefeuille Delta (selection par notes) : {summary['nb_open']} positions "
