"""Bot#29: same conviction-averaging mechanics as simulate_delta_portfolio.py (Bot#25, "Delta"
base), except each new candidate is passed through news_filter.news_verdict() right before it
would actually be bought -- the same gate Bot#5 applies on top of Bot#2's mechanics (see
simulate_constrained_portfolio_newsgated.py). A candidate flagged by the local Ollama model is
skipped this run, free to come up again next run. Customer-concentration sizing (shrinks the
position rather than vetoing it) is applied too, same as Bot#5.

Only fill_slots() (which decides what to buy) and the path-coupled load/save/summary functions
are duplicated here -- reinforce_convictions, recheck_and_exit, FX/fractional-share handling are
all reused directly from simulate_delta_portfolio.py.
"""
# Alias lisibilite (mapping perso) : D5 -- famille Delta (conviction), variante + filtre actu Ollama
import json
import math
import pathlib
import sys
...
LEDGER_PATH = HERE / "results/simulation/delta_portfolio_ledger_newsgated.csv"
STATE_PATH = HERE / "results/simulation/delta_state_newsgated.json"
...
SUMMARY_PATH = HERE / "results/simulation/delta_summary_newsgated.json"
EQUITY_CURVE_PATH = HERE / "results/simulation/delta_equity_curve_newsgated.csv"
...
def write_summary(ledger: pd.DataFrame, cash: float):
    ...
    print(f"\n=== Portefeuille Delta (news-gated) : {summary['nb_open']} positions "
