"""Bot#5: same capital-constrained paper-trading mechanics as
simulate_constrained_portfolio.py (Bot#2 -- capital, slots, sector cap, NA cap, FX and
fractional-share handling all identical), except each candidate is passed through
news_filter.news_verdict() right before it would actually be bought. A candidate flagged
by the local Ollama model as having an obvious news red flag is treated like any other
rejection (unaffordable, fetch failed, ...): skipped this run, free to come up again next
run. Everything else -- exits, FX, fractional-share eligibility, diversification caps -- is
reused directly from simulate_constrained_portfolio.py, so any difference between the two
bots' ledgers isolates the effect of the news gate.

Only fill_slots() (which decides what to buy) and the path-coupled load/save/summary
...
LEDGER_PATH = HERE / "results/simulation/constrained_portfolio_ledger_newsgated.csv"
STATE_PATH = HERE / "results/simulation/constrained_state_newsgated.json"
...
def write_summary(ledger: pd.DataFrame, cash: float):
    ...
    print(f"\n=== Portefeuille contraint (news-gated) : {summary['nb_open']} positions, "
